...
struct gtm_thread
{
  ...
  // The list of all readed and writed bytelock's id.
  vector<size_t> readbytelocks;
  vector<size_t> writebytelocks;
  ...
  // In beginend.cc
  void rollback (bool aborting = false);
  bool trycommit ();
...
extern abi_dispatch *dispatch_ml_wt();
 extern abi_dispatch *dispatch_tlrw();
